...
export default function DeliveryMap({ travellerName, lat, lng, updatedAt, C }: DeliveryMapProps) {
  ...
  return (
    <View style={[styles.mapCard, { backgroundColor: C.surface, borderColor: C.surfaceBorder }]}>
      ...
      <MapView
        style={styles.map}
        region={{ latitude: lat, longitude: lng, latitudeDelta: 0.04, longitudeDelta: 0.04 }}
        scrollEnabled={false}
        zoomEnabled={false}
      >
        <Marker coordinate={{ latitude: lat, longitude: lng }} title={travellerName}>
        ...
        <Circle
          ...
          radius={600}
          fillColor={C.primary + '10'}
          strokeColor={C.primary + '44'}
          strokeWidth={1.5}
        />
      </MapView>

-- Allow both sender and traveller to create requests in create_request_command
...
-- Ensure RLS allows request insert by either sender or traveller
drop policy if exists "requests_insert_participant" on public.requests;
create policy "requests_insert_participant" on public.requests
  for insert to authenticated
  with check (
    auth.uid() = sender_id or auth.uid() = traveller_id
  );

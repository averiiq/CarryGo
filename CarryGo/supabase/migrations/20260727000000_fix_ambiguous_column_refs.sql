-- Fix: "column reference is ambiguous" in transition_request_status
-- The RETURNS TABLE output parameter names (id, status, parcel_id, etc.) conflict
-- with unqualified column names in UPDATE WHERE clauses. Fix by table-qualifying all
-- column references.
...
begin
  ...
  if p_next_status = 'accepted' then
    ...
    update public.trips t2
    set available_capacity = t2.available_capacity - v_parcel.weight
    where t2.id = v_trip.id;

    update public.parcels p2
    set status = 'matched'
    where p2.id = v_parcel.id;
    ...
    set status = 'released',
        released_at = now()
    where pay.request_id = v_request.id
      and pay.status = 'locked';

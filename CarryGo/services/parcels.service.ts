...
export async function fetchParcels(filters?: {
  ...
  if (filters?.userCity && !filters.fromCity && !filters.toCity) {
    try {
      const { fetchCityMarketplaceParcels } = require('@/services/marketplace-relevance.service');
      const cityRes = await fetchCityMarketplaceParcels(filters.userCity, limit, offset);

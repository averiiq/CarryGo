...
export default function CustomerActivityPage() {
  ...
  const loadData = async () => {
    ...
    setTrips(tripsData || [])
    setRequests((reqData as BookingRequest[]) || [])
  ...
  const handleUpdateRequestStatus = async (requestId: string, newStatus: 'accepted' | 'declined') => {
    const supabase = createClient()
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200/80 pb-6">
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-600">
              Track your outgoing packages, manage incoming trip requests, and view escrow transactions.
            </p>
          </div>
          ...
          <div className="mt-6 space-y-4">
            {parcels.length === 0 ? (
              ...
              parcels.map((parcel) => {
                ...
                const acceptedReq = parcelReqs.find((r) => r.status === 'accepted')
                const hasPayment = acceptedReq && payments.find((p) => p.request_id === acceptedReq.id)
                ...
                return (
                  ...
                  >
                    ...
                    {/* Booking Request Status for this Parcel */}
                    {parcelReqs.length > 0 && (
                      <div className="rounded-xl bg-slate-50 p-3.5 space-y-2 border border-slate-200/80">
                        ...
                        {parcelReqs.map((req) => (
                          <div
                            key={req.id}
                            className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2.5 rounded-xl bg-white border border-slate-200 text-xs"
                          >
                            <div className="space-y-0.5">
                              <span className="font-bold text-slate-900">Traveler: {req.traveller_name}</span>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {req.status === 'accepted' && !hasPayment && (
                                <Link
                                  href={`/payment/${req.id}`}
                    ...
                    {tripReqs.length > 0 ? (
                      <div className="rounded-xl bg-slate-50 p-3.5 space-y-2 border border-slate-200/80">
                        ...
                        {tripReqs.map((req) => (
                          ...
                          >
                            <div className="space-y-1">
                              <span className="font-bold text-slate-900">Sender: {req.sender_name}</span>
            ...
            <div className="p-4 rounded-2xl bg-emerald-50/80 border border-emerald-200 flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-emerald-800 leading-relaxed">
                  Every payment is locked safely in RBI-compliant escrow. Funds are never released to the traveler until the recipient enters the verified OTP at delivery.
                </p>
              </div>
            </div>

      ...
      <ScrollLinkedSection className='px-4 sm:px-6 pb-24 md:pb-28'>
            ...
            {trips.map((trip) => (
              <article key={trip.id} className='glass-card rounded-2xl p-4'>
                ...
                <RequestIntentForm
                  mode='trip'
                  tripId={trip.id}
                  suggestedPrice={trip.price_per_kg}
            ...
            {parcels.map((parcel) => (
              <article key={parcel.id} className='glass-card rounded-2xl p-4'>
                ...
                <RequestIntentForm
                  mode='parcel'
                  parcelId={parcel.id}
                  suggestedPrice={parcel.price_offer}
        ...
        <section className='mx-auto mt-7 flex w-full max-w-6xl flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-2xl border border-border bg-surface-elevated/70 px-4 py-3'>
          <div className='inline-flex items-center gap-2 text-sm text-muted'>
            ...
            <span>Verified Travel Guarantee: Dual-OTP handover at pickup & dropoff with SafeVault™ escrow protection.</span>
          </div>
          <Link href='/safety' className='inline-link'>
            Read safety policy
